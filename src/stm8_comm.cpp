...
static void stm8TxByte(uint8_t val)
{
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        if (val & 1u)
        {
            pinMode(MUX_SIG, INPUT); // release → 10 kΩ pulls to 5 V
        }
        else
        {
            pinMode(MUX_SIG, OUTPUT);
...
static bool stm8RxByte(uint8_t *out, uint32_t timeoutUs)
{
    ...
    taskENTER_CRITICAL();
    delayMicroseconds(b / 2u); // Chỉ nhảy 0.5 bit để kiểm tra Start-bit

    // Kiểm tra lại Start-bit (Double check)
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        delayMicroseconds(b); // Nhảy 1 bit để vào giữa bit data
...
{
    pinMode(MUX_SIG, INPUT); // Ensure line is released for STM8 to drive
